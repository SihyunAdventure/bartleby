...
### 2.2 [2차] 19 섹션 매핑 표 (autoplan 2026-05-07 — 14→19 확장; 2026-05-08 ship/polish split)

매핑 표는 [`GALLERY.md`](./GALLERY.md) 에 단일 진실의 원본. 모든 디자인 작업 직후 GALLERY.md 의 Status · Day · Gate 컬럼 update.

#### Two-tier gate (2026-05-08 split — Day 13 review reframe)

19 row 를 두 단계로 나눔. 단일 hard gate (전부 ✅) 는 *sunk-cost trap* — production rendering 에 직접 사용되지 않는 marketing/icon/voice library 컴포넌트가 v1 ship 을 막아서는 안 됨.

- **Ship gate (12 sections)** — production rendering path 에 직접 사용. **Phase 0 acceptance + v1 launch prerequisite**.
  포함: §01 Color · §02 Typography · §03 Spacing · §04 Buttons · §05 Form controls · §06 Recording state · §09 Sidebar · §11 Applied screens · §14 WatchOverlay · §15 Mode Switch · §16 Settings UI · §17 Permission Lifecycle.
- **Polish gate (7 sections)** — Phase 6 marketing prep / 후속 polish.
  포함: §00 Manifesto · §07 Badges · §08 Domain blocks · §10 Voice library · §12 App icon · §13 Marketing · §18 Marketing Hero (Watch reframe).

**Sacred Values (§4.2) 는 *모든* row 에서 enforce.** Gate split 은 *완료 acceptance 범위* 만 좁힘 — voice copy 정확성 / OKLCH chroma 0 / 폰트 5종 stack 은 polish gate row 에서도 지켜야 함.

**예외**: §10 Voice library 는 polish gate 지만 *voice strings 자체* 는 production 의 모든 toast/error/empty state 에서 inline 사용. 즉 *컴포넌트 갤러리* 만 polish 로 미루고, 문구는 §3.1 anti-pattern 으로 ship 전에 inline 정확.

#### 완료 정의
- **Phase 0 acceptance** = ship gate 12 row 전부 ✅
- **v1 launch acceptance** = ship gate 12 row 전부 ✅ (polish gate 는 v1.x 후속)
- **Phase 6 acceptance** = 19 row 전부 ✅

#### 추가 acceptance per row (dual-mode)
- §04 / §05 / §06 (Buttons / Forms / Recording state) — *시청 모드 + 미팅 모드 양쪽 state* 검증
- §10 (Voice library) — *Watch mode 전용 voice copy matrix* (idle/listening/captioning/translating/reconnecting/drm-blocked/permission-denied/complete) 도 ✅
- §13 / §18 (Marketing) — *Watch hero* 가 primary, meeting hero 가 secondary (§18 이 §13 의 reframe)
- §14 (WatchOverlay) — `design-system-extensions/overlay.md` spec 의 모든 state · token · voice 일치
- §16 (Settings) — `design-system-extensions/settings.md` spec 의 5 탭 모두 구현
