...
### Day 19b 결과 (§11 LiveCaption gallery + overlay voice polish ✅ — code path)

A1 chunk 잔여. §11 LiveCaption gallery section + App.tsx Overlay voice copy/transition 소규모 polish.

**구조 (4 파일 수정/신규)**:
- `src/gallery/sections/11-LiveCaption.tsx` (신규, ~460 lines) — `LiveCaption` 컴포넌트:
  - **DSSection wrapper** — `id="livecaption"`, `kicker="11 · LiveCaption"`, title "Caption surface spec.", lede (한국어 first 자막 설명)
  - **caption_mode 3 모드 비교** — `CaptionMock` primitive: KO only / KO+EN (default) / EN only. 동일한 EN+KO 샘플 텍스트로 side-by-side 비교.
  - **overlay_opacity 4 frame** — bgAlpha 60/75/85/100%. `CaptionMock` 반복.
  - **caption_font_size 5 frame** — koFontSize 14/15/16/17/18px, 16px default 명시. 세로 스택.
  - **Caption state matrix 7 mock** — awaiting / drm-blocked / stt-error / partial (EN streaming) / final+KO partial (mid-stream) / translation-error / all-flowing (happy path). `StateMock` + `CaptionMock` 혼용.
  - **Spec mapping memo** — 12 spec row 표: EN layer 11px / rgba(40,40,40,0.55) / lh 1.35 / minHeight 1.35em, KO layer prefs px / rgba(15,15,15,0.92) / lh 1.45 / minHeight 1.45em, KO partial opacity 0.65, container rgba(248,247,244,bgAlpha) blur(12px) borderRadius 6 padding 10/14 gap 4. **전 항목 ✓ (drift 없음)**.
- `src/gallery/Gallery.tsx` — `LiveCaption` import + `<LiveCaption />` render
- `src/gallery/Gallery.module.css` — §11 CSS 블록: `.caption-mock-wrap` / `.caption-mock-label` / `.caption-mock-note` / `.caption-mock-frame` / `.caption-mode-row` (3-col) / `.caption-opacity-row` (4-col) / `.caption-fontsize-col` (flex col) / `.caption-state-grid` (2-col grid) / `.spec-table` / `.spec-table-head` / `.spec-table-row`
- `src/App.tsx` — Overlay polish 3종:
  - `sttError` voice: `"STT: {sttError}"` → `"Bartleby would prefer not to. ({sttError.slice(0,100)})"` italic + `fontFamily: "var(--font-serif)"`
  - `translationError` voice: `"번역 대기 중… ({translationError})"` → `"번역 대기 중…"` (error string hidden) italic + Cormorant 12px + rgba(40,40,40,0.55)
  - KO partial span: `transition: "opacity 200ms ease"` 추가 (opacity 0.65 유지)

**Overlay polish 결정**:
- translation_error simplification: **적용** — error 이유 노출은 UX noise. italic Cormorant 톤이 voice에 어울림.
- stt_error Cormorant: **적용** — "Bartleby would prefer not to." literary 톤 + Cormorant serif 적합. 100자 슬라이스로 overflow 방지.
- KO partial 200ms transition: **적용** — 200ms 는 token 단위 streaming (100–200ms interval) 과 같은 스케일이므로 jank 없이 부드러움. 사용자 "더 느려진거 같다" 피드백은 Day 16b throttle 시도 (translation 레벨) 였고 opacity transition 은 다른 레이어이므로 risk 낮음.

**Spec drift**:
- 없음. spec mapping memo 12 rows 전부 ✓. `borderRadius: 6` / `gap: 4` unitless 는 React → px 자동 해석 확인.

**검증**:
- ✅ `pnpm build` (tsc + vite) — 0 TypeScript errors. 63 modules transformed.
- ✅ `cargo build --manifest-path src-tauri/Cargo.toml` — Finished dev (0.13s, no Rust changes)
- ✅ `cargo test --manifest-path src-tauri/Cargo.toml --lib` — 33 passed; 0 failed; 1 ignored (0.41s)
- ✅ Linear SIH-1187 "A3 §11 LiveCaption gallery + overlay polish" — created + Done (https://linear.app/sihyun-dev/issue/SIH-1187/a3-11-livecaption-gallery-overlay-polish)
- ✅ Commit `8101398`

**Day 19b scope (의도적 제외 — 후속)**:
- §03~§10, §12~§15, §17~§18 gallery sections — Phase 0 lazy fill.
- Overlay KO partial transition WebView 실제 렌더 검증 — dogfood 시 visual verify.
- `var(--font-serif)` CSS custom property 가 Overlay WebView 컨텍스트에서 Cormorant Garamond 로 resolve 되는지 dogfood 시 확인 (tokens.css import 경로 의존).
...
### Day 20a 결과 (chunk B Meeting Mode UI 본진 ✅ — code path)

Phase 4 진입. Watch 모드 기존 동작 유지. Meeting 모드 진입 시 sidebar 240px + main grid + transcript view + recording controls. capture 인프라 재사용 (start_capture / stop_capture / stt_final / translation_final). from-scratch (memory rule: external reference 코드 복사 금지, 패턴만 학습).

**구조 (11 파일 신규/수정)**:
- `src/types/capture.ts` (신규) — `CaptureStats` / `DriftStats` / `RssStats` 공유 타입 추출. App.tsx + Meeting 컴포넌트 양쪽에서 import.
- `src/meeting/Meeting.tsx` + `.module.css` (신규) — sidebar + main 2-column grid shell. `captureRunning` / `lastStats` / `clearToken` state 소유. `onStart` 시 `clearToken` increment → TranscriptView clear signal. sidebar + transcript + recording controls 세 자식 조율.
- `src/meeting/Sidebar.tsx` + `.module.css` (신규) — 240px, `var(--paper-2)` bg, `var(--rule)` right border. bartleby logo (display 18px) + Cormorant italic epigraph + Mode Segmented control + ⚙ Settings button + divider + Status block (Idle/Recording…/Stopped, drift_ms, peak_dbfs, rss peak — stop_capture stats 기반).
- `src/meeting/TranscriptView.tsx` + `.module.css` (신규) — scrolling utterance list. `stt_final` + `translation_final` listen. `clearToken` prop으로 부모 Start 시 초기화. 화자 분리 mock: language "ko" → user pill (ink bg), 기타 → system pill (paper-3 bg). KO translation → italic Gowun Batang second line. MAX 100 utterances retain. 빈 상태: "Bartleby is listening, would prefer not to speak." italic Cormorant.
- `src/meeting/RecordingControls.tsx` + `.module.css` (신규) — Start / Stop buttons (`btn` / `btn-primary` 재사용). `invoke("start_capture")` / `invoke<CaptureStats>("stop_capture")`. recording 중 red dot + "Recording…" indicator.
- `src/App.tsx` — `Meeting` import. CaptureStats 타입을 `src/types/capture.ts` 로 이동. `watchShell` JSX variable (함수 내부 컴포넌트 정의 회피 — remount 방지). `appMode === "meeting"` 분기: `<Meeting>` 렌더. Settings modal 양 모드 모두 렌더.
- `src/App.css` — `.container[data-mode="meeting"] { padding: 0 }` 채움. `.meeting-sidebar-placeholder` 제거.

**설계 결정**:
- `watchShell` = JSX variable (const) 로 정의 — `function WatchShell()` 를 App() 내부에 두면 React 가 render 마다 새 컴포넌트 타입으로 인식해 remount 발생. JSX variable 은 안전.
- transcript: final 만 표시 (over-engineer 회피 — partial 표시는 Meeting 에서 별도 결정 필요).
- 화자 분리: language field 기반 임시 (`ko` = user mic 추정, 기타 = system). mic 실작동 후 재정의.
- mode 전환 시 capture: 그대로 (사용자가 명시적으로 Stop). 자동 stop X.
- `src/types/capture.ts` 신규 (1 파일) — Meeting + Watch 양쪽이 동일 CaptureStats 필요. App.tsx 에서 re-export 하면 Meeting → App 의존 생겨 역방향. 공유 types 파일이 가장 깔끔.

**검증**:
- ✅ `pnpm build` (tsc + vite) — 0 TypeScript errors, 430ms. 73 modules transformed.
- ✅ `cargo build --manifest-path src-tauri/Cargo.toml` — Finished dev (0.13s, no Rust changes)
- ✅ `cargo test --manifest-path src-tauri/Cargo.toml --lib` — 33 passed; 0 failed; 1 ignored (0.41s)
- ✅ Commit `4502d03`

**Day 20a scope (의도적 제외 — 후속)**:
- Mic 실 캡처 — Apple Dev ID + signed build 후. 현재 system audio only (mic_buffers_received = 0 예상). 화자 분리 재정의 시점.
- transcript partial 표시 — Meeting 모드 에서 partial 지시자 필요 시 별도 결정.
- mode 전환 시 capture 자동 stop — 현재 그대로 (사용자 명시 Stop). UX 검증 후 결정.
- fade transition `opacity 200ms` 모드 전환 — chunk B+ 에서 sidebar mount/unmount 와 함께.
- Meeting 모드 에서 overlay 처리 — Option A (storage-only) 유지. Meeting 진입 시 overlay hide/show 는 Phase 4+ 에서.
